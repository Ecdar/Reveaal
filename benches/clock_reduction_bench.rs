...
fn clock_reduced_refinement() {
    ...
    let mut loader =
        JsonProjectLoader::new_loader("samples/json/EcdarUniversity".to_string(), DEFAULT_SETTINGS)
            .to_comp_loader();
...
fn normal_refinement() {
    let query = parse_to_query(QUERY);
    let mut loader =
        JsonProjectLoader::new_loader("samples/json/EcdarUniversity".to_string(), TEST_SETTINGS)
            .to_comp_loader();
    let executor = create_executable_query(query.get(0).unwrap(), &mut *loader).unwrap();
    executor.execute();
}
