...
#undef TEMPLATE
    ...
    {
        assert(!isEmpty());
        return dbm_writeToMinDBMWithOffset(const_dbm(), pdim(), minimizeGraph, tryConstraints16,
                                           c_alloc, offset);
    }
    ...
    {
        assert(!isEmpty());
        return dbm_writeAnalyzedDBM(const_dbm(), pdim(), bitMatrix, nbConstraints, tryConstraints16,
                                    c_alloc, offset);
    }
