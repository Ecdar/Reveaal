...
impl CompiledUpdate {
    pub fn compile(update: &parse_edge::Update, decl: &Declarations) -> Result<CompiledUpdate> {
        match update.get_expression() {
            BoolExpression::Int(val) => {
                if let Ok(clock_index) = decl.get_clock_index_by_name(update.get_variable_name()) {
                    Ok(CompiledUpdate {
                        clock_index,
                        value: *val,
                    })
