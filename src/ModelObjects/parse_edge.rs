...
pub fn parse(edge_attribute_str : &str) -> Result<EdgeAttribute, Error<Rule>>{
    let mut pairs = EdgeParser::parse(Rule::edgeAttribute, edge_attribute_str).unwrap_or_else(|e| panic!("Could not parse as rule with error: {}", e));
    let pair = pairs.next().unwrap();
    match pair.as_rule() {
        Rule::edgeAttribute => {
            Ok(build_edgeAttribute_from_pair(pair))
        }
        err => {panic!("Unable to match edgeAttribute string as rule: {:?}", err)}
    }
}
pub fn build_edgeAttribute_from_pair(pair: pest::iterators::Pair<Rule>) -> EdgeAttribute{
    ...
    match pair.as_rule() {
        Rule::update => {
            build_update_from_pair(pair)
        },
        Rule::guard => {
            build_guard_from_pair(pair)
        }
        err => {panic!("Unable to match update string as rule: {:?}", err)}
    }
}
    ...
    match pair.as_rule() {
        Rule::assignments => {
            ...
            match assignment_pair.as_rule() {
                ...
                err => panic!("Unable to match: {:?} as rule assignment or finalAssignment", err),
            }                 
        }
        unknown_pair => panic!("Tried to match pair as assignment, but it was {:?}", unknown_pair)
    }
