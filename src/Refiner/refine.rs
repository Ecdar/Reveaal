...
pub fn refines(
    ...
    if let Some(inputs2) = sys_decls
        .get_declarations()
        .get_input_actions()
        .get(machine2.get_name())
    {
        if let Some(outputs1) = sys_decls
            .get_declarations()
            .get_output_actions()
            .get(machine1.get_name())
        {
            let initial_locations_1: Vec<&component::Location> = machine1
                .get_locations()
                .into_iter()
                .filter(|location| {
                    location.get_location_type() == &component::LocationType::Initial
                })
                .collect();
            let initial_locations_2: Vec<&component::Location> = machine2
                .get_locations()
                .into_iter()
                .filter(|location| {
                    location.get_location_type() == &component::LocationType::Initial
