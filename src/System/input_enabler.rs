...
pub fn make_input_enabled(automaton: &mut Automaton, inputs: &[String]) {
    let dimension = automaton.declarations.get_clock_count() + 1;
    let mut new_edges: Vec<Edge> = vec![];
    let input_edges = automaton
        .get_edges()
        .iter()
    ...
    for location in automaton.get_locations() {
        ...
        for input in inputs {
            ...
            for edge in specific_edges {
                ...
                if let Some(target_invariant) = automaton
                ...
                if let Some(updates) = edge.get_update() {
                    for update in updates {
                        let cu = update.compiled(automaton.get_declarations());
            ...
            new_edges.push(Edge {
                ...
                guard: BoolExpression::from_disjunction(
                    ...
                    automaton.get_declarations().get_clocks(),
                ),
                update: None,
    ...
    automaton.add_edges(&mut new_edges);
}
