...
impl Composition {
    pub fn new(
        ...
        if !left_out.is_disjoint(&right_out) {
            return bail!("Invalid parallel composition, outputs are not disjoint");
        }
...
impl TransitionSystem for Composition {
    ...
    fn next_transitions(&self, location: &LocationTuple, action: &str) -> Result<Vec<Transition>> {
        ...
        if self.common_actions.contains(action) {
            ...
            return Ok(Transition::combinations(
                &left,
                &right,
                CompositionType::Composition,
            ));
    ...
    fn is_locally_consistent(&self) -> Result<bool> {
        Ok(self.left.is_locally_consistent()? && self.right.is_locally_consistent()?)
    }
