...
impl Quotient {
    pub fn new(
        ...
        if !T.precheck_sys_rep()? {
            bail!("T (left) must be least consistent for quotient");
        }

        if !S.precheck_sys_rep()? {
            bail!("S (right) must be least consistent for quotient");
        }
...
impl TransitionSystem for Quotient {
    ...
    fn is_deterministic(&self) -> Result<bool> {
        Ok(self.T.is_deterministic()? && self.S.is_deterministic()?)
    }

    fn is_locally_consistent(&self) -> Result<bool> {
        Ok(self.T.is_locally_consistent()? && self.S.is_locally_consistent()?)
    }
