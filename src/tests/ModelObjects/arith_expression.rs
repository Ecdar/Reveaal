#[cfg(test)]
mod arith_expression {
    use crate::ModelObjects::representations::{ArithExpression as AE, ArithExpression};
    use AE::*;
    #[test]
    fn simplify_test1() {
    ...
    fn simplify_test5() {
        //((5-4)-3)-2
        let mut expr = AE::ADif(AE::ADif(AE::ADif(Int(5), Clock(4)), Int(3)), Int(2));
        assert_eq!(Ok(AE::ADif(Int(0), Clock(4))), expr.simplify());

        let mut expr = AE::AAdd(AE::AAdd(AE::AAdd(Int(5), Clock(4)), Int(3)), Int(2));
    ...
    fn simplify_test6() {
        //5-(4-(3-2))
        let mut expr = AE::ADif(Int(5), AE::ADif(Int(4), AE::ADif(Int(3), Clock(2))));
        assert_eq!(Ok(AE::ADif(Int(4), Clock(2))), expr.simplify());

        let mut expr = AE::AAdd(Int(5), AE::AAdd(Int(4), AE::AAdd(Int(3), Clock(2))));
    ...
    fn simplify_test_highoperators_ints() {
        let mut expr = AE::AMul(Int(10), Int(5));
        assert_eq!(Ok(Int(50)), expr.simplify());

        let mut expr = AE::ADiv(Int(10), Int(5));
        assert_eq!(Ok(Int(2)), expr.simplify());

        let mut expr = AE::AMod(Int(10), Int(5));
        assert_eq!(Ok(Int(0)), expr.simplify());
    }

    #[test]
    fn simplify_test_highoperators_clocks() {
        let mut expr = AE::AMul(Clock(10), Int(5));
        assert_eq!(expr.simplify().ok(), None);

        let mut expr = AE::AMul(Int(10), Clock(5));
        assert_eq!(expr.simplify().ok(), None);

        let mut expr = AE::ADiv(Clock(10), Int(5));
        assert_eq!(expr.simplify().ok(), None);

        let mut expr = AE::ADiv(Int(10), Clock(5));
        assert_eq!(expr.simplify().ok(), None);

        let mut expr = AE::AMod(Clock(10), Int(5));
        assert_eq!(expr.simplify().ok(), None);

        let mut expr = AE::AMod(Int(10), Clock(5));
        assert_eq!(expr.simplify().ok(), None);
    }
}
