...
use crate::xml_parser::{parse_xml_comp_from_str, parse_xml_from_file, parse_xml_from_str};
...
fn compare_components(mut c1: &Component, mut c2: &Component) {
    assert_eq!(c1.name, c2.name);
    println!("{:?}", c1);

    assert_eq!(c1.edges.len(), c2.edges.len());
    for (i, mut e) in c1.edges.iter().enumerate() {
