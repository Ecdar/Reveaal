...
mod Conjunction_refinement {
    ...
    fn test1NestedConjRefinesT5() {
        ...
        assert!(refine::check_refinement(ts2, SystemRepresentation::Component(automataList.get(4).unwrap().clone()), decl.borrow()).unwrap());
    }
    ...
    fn test1NestedConjRefinesT12() {
        ...
        let ts2: SystemRepresentation = SystemRepresentation::Conjunction(Box::from(ts1),
                                                                          Box::from(SystemRepresentation::Component(automataList.get(10).unwrap().clone())));

        assert!(refine::check_refinement(ts2, SystemRepresentation::Component(automataList.get(11).unwrap().clone()), decl.borrow()).unwrap());
    }
}